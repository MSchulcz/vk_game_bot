...
''')  # после первого запуска требуется удалить этот кусок кода, так как он нужен один лишь раз
    ...
    try:
        ...
        if messages["count"] >= 1:
            ...
            if body.lower() == "начать" or body.lower() == "помощь":
                ...
                &#127968;Дом - 2(100000$)
                &#128664;Машина - 3(50000$)
                🏡Дача - 4(40000$)""", "random_id": random.randint(1, 2147483647)})
            elif "работа" in body.lower():
                cube = random.randint(1, 6)
                user_cube = str(body.lower()[-1])
                user_win = random.randint(1, 200)
                connection = sql.connect("user.sqlite", check_same_thread=False)
                q = connection.cursor()
                q.execute("SELECT * FROM user_info WHERE User_ID = %s" % (id))
                result = q.fetchall()
                if len(result) == 0:
                    ...
                    vk.method("messages.send", {"peer_id": id,
                                                "message": user_name + ", вы заработали:" + str(
                    ...
                    try:
                        ...
                        if result[0][3] >= rate:
                            if kazino == 1:
                                ...
                                connection = sql.connect('user.sqlite', check_same_thread=False)
                                q = connection.cursor()
                                q.execute("UPDATE user_info SET Balance = '%s' WHERE User_ID = '%s'" % (
                                result[0][3] - rate, id))
                                connection.commit()
                                connection.close()
                        else:
                            ...
                            if money >= 1000:
                                if ownment != None:
                                    q.execute("UPDATE user_info SET Ownment = '%s' WHERE User_ID = '%s'" % (
                                        str(ownment) + "1,", id))
                                    q.execute(
                                        "UPDATE user_info SET Balance = '%s' WHERE User_ID = '%s'" % (money - 10000,
                                                                                                      id))
                                    connection.commit()
                                    connection.close()
                                    vk.method("messages.send",
                                              {"peer_id": id, "message": "Вы купили телефон&#128241;;!",
                                               "random_id": random.randint(1, 2147483647)})
                                else:
                        ...
                        if realty == 2:
                            ...
                            if money >= 10000:
                                if ownment != None:
                                    ...
                                    connection.commit()
                                    connection.close()
                                    vk.method("messages.send", {"peer_id": id, "message": "Вы купили палатку&#9978;!",
                                                                "random_id": random.randint(1, 2147483647)})
                                else:
